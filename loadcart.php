<?php
error_reporting(0);
include_once ("dbconnect.php");
$email = $_POST['email'];

if (isset($email)){
     $sql = "SELECT tbl_products.prid, tbl_products.prname, tbl_cart.prprice, tbl_cart.prqty FROM tbl_products INNER JOIN tbl_cart ON tbl_cart.prid = tbl_products.prid WHERE tbl_cart.user_email = '$email'";
}


$result = $conn->query($sql);

if ($result->num_rows > 0)
{
    $response["cart"] = array();
    while ($row = $result->fetch_assoc())
    {
        $cartlist = array();
        $cartlist["prid"] = $row["prid"];
        $cartlist["prname"] = $row["prname"];
        $cartlist["prprice"] = $row["prprice"];
        $cartlist["prqty"] = $row["prqty"];
        
        $cartlist["total_price"] = round(doubleval($row["prprice"])*(doubleval($row["prqty"])),2)."";
        
        array_push($response["cart"], $cartlist);
    }
    echo json_encode($response);
}
else
{
    echo "Cart Empty";
}

?>